...
class _QuestionPageState extends State<QuestionPage> {
  ...
  // NOTIFIKASI ATAS
  // =============================================
  void _showTopSnackBar(String message, Color color, {IconData icon = Icons.info_outline}) {
  ...
  Future<void> simpanSementara() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString(draftKey, jsonEncode({
      "answers": answers.map((k, v) => MapEntry(k.toString(), v)),
      "saved_at": DateTime.now().toIso8601String(),
    }));
    if (!mounted) return;
    _showTopSnackBar('Jawaban disimpan sementara', const Color(0xFF1565C0), icon: Icons.save_outlined);
  }
  ...
  Future<void> loadDraft() async {
    ...
    final typeById = {for (final q in questions) q['id'].toString(): q['type']?.toString() ?? ''};
    ...
    setState(() {
      ...
      savedAnswers.forEach((key, value) {
        ...
        if (qType == 'multiple') {
          ...
          answers[rKey] = (value is List) ? (value.isNotEmpty ? value.first.toString() : null) : value?.toString();
        } else {
          answers[rKey] = value;
        }
      });
  ...
  List<dynamic> _parseMultipleValue(dynamic value) {
    ...
    if (value is String && value.isNotEmpty) {
      try { final d = jsonDecode(value); if (d is List) return List<dynamic>.from(d); } catch (_) {}
    }
    return [];
  }
  ...
  String _getAnswerByKode(String kode) {
    final q = questions.firstWhere((q) => q['kode_soal']?.toString() == kode, orElse: () => {});
    if (q.isEmpty) return '';
  ...
  bool _isAnswered(Map<String, dynamic> q) {
    ...
    if (type == 'matrix') {
      ...
      return details.every((d) {
        ...
        return answers.containsKey(mapKey) && answers[mapKey] != null && answers[mapKey].toString().isNotEmpty;
      });
    }
  ...
  bool shouldShow(Map<String, dynamic> q) {
    ...
    if (['f502','f505','f5a1','f5a2','f1101','f1102','f5b','f5d','f6','f7','f7a'].contains(kode)) return status.contains('Bekerja');
    if (['f503','f5c'].contains(kode)) return status.contains('Wiraswasta');
    if (['f18a','f18b','f18c','f18d'].contains(kode)) return status.contains('Melanjutkan Pendidikan');
    if (kode == 'f302') return _getAnswerByKode('f301').contains('sebelum lulus');
    if (kode == 'f303') return _getAnswerByKode('f301').contains('sesudah lulus');
    if (kode == 'f1002') return _getAnswerByKode('f1001').contains('Lainnya');
    if (kode == 'f416') return _getAnswerByKode('f401-f416').contains('Lainnya');
    if (kode == 'f1102') return _getAnswerByKode('f1101').contains('Lainnya');
    if (kode == 'f1202') return _getAnswerByKode('f1201').contains('Lainnya');
    if (kode == 'f1614') return _getAnswerByKode('f1601-f1614').contains('Lainnya');
    return true;
  }

  void _resetConditionalAnswers() {
    for (final kode in ['f502','f503','f505','f5a1','f5a2','f1101','f1102','f5b','f5c','f5d','f18a','f18b','f18c','f18d']) {
      final q = questions.firstWhere((q) => q['kode_soal']?.toString() == kode, orElse: () => {});
  ...
  Widget buildInput(Map<String, dynamic> q) {
    ...
    if (type == 'text' && dataType == 'date') {
      ...
      return InkWell(
        onTap: () async {
          ...
          if (picked != null) setState(() => answers[id] = picked.toIso8601String().split('T')[0]);
        },
        borderRadius: BorderRadius.circular(10),
        child: Container(
          ...
          decoration: BoxDecoration(
            color: Colors.white,
            borderRadius: BorderRadius.circular(10),
            border: Border.all(color: hasValue ? _kPrimary : Colors.grey.shade300),
          ),
          child: Row(children: [
            Icon(Icons.calendar_today_outlined, size: 18, color: hasValue ? _kPrimary : Colors.grey.shade400),
            const SizedBox(width: 10),
            Text(
              hasValue ? answers[id].toString() : 'Pilih tanggal',
              style: TextStyle(fontSize: 14, color: hasValue ? Colors.black87 : Colors.grey.shade400),
            ),
          ]),
        ),
      );
              ...
              child: Row(children: [
                AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(color: isSelected ? _kPrimary : Colors.grey.shade400, width: 2),
                    color: isSelected ? _kPrimary : Colors.transparent,
                  ),
                  child: isSelected ? const Icon(Icons.check, size: 12, color: Colors.white) : null,
                ),
                const SizedBox(width: 12),
                Expanded(child: Text(label, style: TextStyle(fontSize: 14, color: isSelected ? _kPrimary : Colors.black87, fontWeight: isSelected ? FontWeight.w600 : FontWeight.normal))),
              ]),
            ),
          );
        }).toList(),
          ...
          return GestureDetector(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                color: isSelected ? _kPrimary.withOpacity(0.06) : Colors.white,
                borderRadius: BorderRadius.circular(10),
                border: Border.all(color: isSelected ? _kPrimary : Colors.grey.shade300, width: isSelected ? 1.5 : 1),
              ),
              child: Row(children: [
                AnimatedContainer(
                  duration: const Duration(milliseconds: 150),
    ...
    if (type == 'scale') {
      ...
      final scaleColors = [Colors.red.shade300, Colors.orange.shade300, Colors.amber.shade400, Colors.lightGreen.shade400, Colors.green.shade500];
      return Column(
        children: [
          Row(
              ...
              return GestureDetector(
                ...
                child: AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(color: isSelected ? scaleColors[i] : Colors.grey.shade300, width: isSelected ? 0 : 1),
                    boxShadow: isSelected ? [BoxShadow(color: scaleColors[i].withOpacity(0.4), blurRadius: 8, offset: const Offset(0, 3))] : [],
                  ),
                  child: Center(child: Text(val, style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: isSelected ? Colors.white : Colors.grey.shade500))),
                ),
              );
            }),
          ),
          ...
          if (answers[id] != null) ...[
            ...
            Center(
              child: Container(
                ...
                decoration: BoxDecoration(color: scaleColors[int.parse(answers[id].toString()) - 1].withOpacity(0.15), borderRadius: BorderRadius.circular(20)),
                child: Text(
                  scaleLabels[int.parse(answers[id].toString()) - 1],
                  textAlign: TextAlign.center,
                  style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: scaleColors[int.parse(answers[id].toString()) - 1]),
                ),
              ),
            ),
          ],
          ...
          children: [
              ...
              child: Row(children: [
                ...
                ...List.generate(5, (i) => Expanded(
                  ...
                  child: Column(children: [
                    Text('${i + 1}', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: _kPrimary)),
                  ]),
                )),
              ]),
            ),
            ...
            ...details.asMap().entries.map<Widget>((entry) {
              ...
              return Container(
                ...
                child: Row(children: [
                  Expanded(flex: 3, child: Text(label, style: const TextStyle(fontSize: 12, color: Colors.black87))),
                  ...List.generate(5, (j) {
                    final val = (j + 1).toString();
                    final isSelected = answers[mapKey]?.toString() == val;
                    final dotColor = [Colors.red.shade300, Colors.orange.shade300, Colors.amber.shade400, Colors.lightGreen.shade400, Colors.green.shade500][j];
                    return Expanded(
                      flex: 1,
                        ...
                        child: Center(
                          child: AnimatedContainer(
                            ...
                            decoration: BoxDecoration(
                              ...
                              boxShadow: isSelected ? [BoxShadow(color: dotColor.withOpacity(0.4), blurRadius: 4, offset: const Offset(0, 2))] : [],
                            ),
  ...
  Future<void> submit() async {
    ...
    try {
      final validIds = questions.map((q) => int.tryParse(q['id'].toString()) ?? 0).where((id) => id > 0).toSet();
      List payload = [];
      ...
      answers.forEach((k, v) {
        ...
        payload.add({'question_id': qId, 'value': v is List ? jsonEncode(v) : v.toString()});
      });
      ...
      answers.forEach((k, v) {
        ...
        payload.add({'question_id': qId, 'value': v.toString(), 'detail_id': detailId});
      });

      if (payload.isEmpty) {
        _showTopSnackBar('Belum ada jawaban untuk dikirim', Colors.orange.shade700, icon: Icons.warning_amber_outlined);
        return;
      }
      ...
      if (res['status'] == true) {
        _showTopSnackBar('Jawaban berhasil dikirim!', const Color(0xFF2E7D32), icon: Icons.check_circle_outline);
      } else {
        _showTopSnackBar(res['message'] ?? 'Gagal mengirim jawaban', const Color(0xFFC62828), icon: Icons.error_outline);
      }
    } catch (e) {
      debugPrint('ERROR SUBMIT: $e');
      if (mounted) _showTopSnackBar('Terjadi kesalahan, coba lagi', const Color(0xFFC62828), icon: Icons.error_outline);
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    final progress = visibleQuestions.isEmpty ? 0.0 : answered / visibleQuestions.length;

    SharedPreferences.getInstance().then((p) => p.setDouble("progress_$userId", progress));

    // Warna progress dinamis
    Color progressColor;
    if (progress < 0.4) progressColor = Colors.red.shade400;
    else if (progress < 0.75) progressColor = Colors.orange.shade400;
    else progressColor = Colors.green.shade500;
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Column(
          children: [
            Text('Tracer Study', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.white)),
            Text('Survei Alumni', style: TextStyle(fontSize: 11, color: Colors.white60)),
          ],
        ),
      ),
      body: isLoading
                ...
                child: Column(children: [
                  Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                    ...
                    Container(
                      ...
                      child: Text('$answered / ${visibleQuestions.length} soal',
                  ...
                  itemBuilder: (ctx, i) {
                    ...
                    return Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: Border(left: BorderSide(color: answered ? Colors.green.shade400 : Colors.grey.shade300, width: 4)),
                        ...
                        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          ...
                          Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                            Container(
                              ...
                              decoration: BoxDecoration(
                                color: answered ? Colors.green.shade400 : _kPrimary.withOpacity(0.08),
                                shape: BoxShape.circle,
                              ),
                              child: Center(child: Text(
                                '${i + 1}',
                                style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: answered ? Colors.white : _kPrimary),
                              )),
                            ),
                            Expanded(child: Text(
                              q['question_text'] ?? '-',
                          ...
                          if (hint.isNotEmpty) ...[
                            ...
                            Container(
                              ...
                              child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                ...
                                Expanded(child: Text(hint, style: TextStyle(fontSize: 12, color: Colors.blue.shade700, fontStyle: FontStyle.italic))),
                              ]),
                            ),
                          ],
                ...
                Expanded(
                  ...
                  child: SizedBox(
                    ...
                    child: ElevatedButton.icon(
                      ...
                      label: Text(_isSubmitting ? 'Mengirim...' : 'Kirim Jawaban',
              ...
              SizedBox(
                ...
                child: TextButton.icon(
                  onPressed: () async {
                    ...
                    setState(() => answers = {});
                    if (!mounted) return;
                    _showTopSnackBar('Draft berhasil dihapus', Colors.grey.shade700, icon: Icons.delete_outline);
                  },
                  icon: const Icon(Icons.refresh, size: 14, color: Colors.grey),
                  label: const Text('Reset Draft', style: TextStyle(color: Colors.grey, fontSize: 12)),
                ),
              ),
            ]),
          ),
